...
const MANAGED_PI_LAUNCHER_PATHS = [
  "dist/cli.js",
  "dist/bundle/cli.js",
] as const;
...
async function importExtensionWithVirtualPeers(
  ...
  if (!existsSync(launcherPath)) {
    fail(`managed Pi launcher is missing: ${launcherPath}`);
  }
  ...
  // Use the regular entry here because Node's require condition does not
  // expose jiti/static. The Pi loader imports jiti/static from ESM, while the
  // same createJiti API and virtualModules behavior are exercised below.
...
async function main(): Promise<void> {
  ...
  const codingAgentRoot = findPackageRoot("@earendil-works/pi-coding-agent");
  const managedLauncherPaths = MANAGED_PI_LAUNCHER_PATHS.map((relativePath) =>
    join(codingAgentRoot, relativePath),
  );
  for (const launcherPath of managedLauncherPaths) {
    if (!existsSync(launcherPath)) {
      fail(`managed Pi launcher is missing: ${launcherPath}`);
    }
  }
  ...
  for (const launcherPath of managedLauncherPaths) {
    for (const entryPath of managedEntries) {
      await activateAndProbe(managedWorkspace, entryPath, (path) =>
        importExtensionWithVirtualPeers(path, launcherPath),
      );
      console.log(`PASS managed activate ${entryPath} via ${launcherPath}`);
    }
  }
